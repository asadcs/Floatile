...
public sealed class DevQAPanel : MonoBehaviour
{
    ...
    static readonly long[] PRESET_TIERS   = { 2L, 4L, 8L, 1_000_000L, 1_000_000_000L, 1_000_000_000_000L };
    static readonly string[] PRESET_LABELS = { "2", "4", "8", "1M", "1B", "1T" };
    ...
    void OnGUI()
    {
        ...
        for (int i = 0; i < PRESET_TIERS.Length; i++)
        {
            if (GUILayout.Button(PRESET_LABELS[i]))
                SetHeroTier(PRESET_TIERS[i]);
        }
        ...
        GUILayout.BeginHorizontal();
        if (GUILayout.Button("Force White")) _spawner?.ForceSpawnWhite();
        if (GUILayout.Button("Force Black")) _spawner?.ForceSpawnBlack();
        GUILayout.EndHorizontal();
